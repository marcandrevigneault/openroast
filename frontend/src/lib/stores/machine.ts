...
export function createInitialState(machineId: string, machineName: string): MachineState {
	return {
		machineId,
		machineName,
		sessionState: 'idle',
 ...
 */
export function processMessage(state: MachineState, msg: ServerMessage): MachineState {
	switch (msg.type) {
		case 'temperature': {
