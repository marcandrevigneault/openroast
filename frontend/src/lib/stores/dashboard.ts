...
export function addMachine(state: DashboardState, machine: DashboardMachine): DashboardState {
	if (state.machines.some((m) => m.id === machine.id)) return state;
	return { ...state, machines: [...state.machines, machine] };
}

export function removeMachine(state: DashboardState, machineId: string): DashboardState {
	return { ...state, machines: state.machines.filter((m) => m.id !== machineId) };
}
