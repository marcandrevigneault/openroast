...
describe('saveProfile', () => {
	...
	it('throws on error response', async () => {
		...
		await expect(
			saveProfile({
				profile: { name: 'x', machine: '', temperatures: [], events: [], controls: {} }
			})
...
describe('listProfiles', () => {
	it('returns profile summaries', async () => {
		const summaries = [{ id: '1', name: 'Test', machine: '', created_at: '', bean_name: '', data_points: 0 }];
		mockFetch.mockResolvedValue({
			ok: true,
...
describe('deleteProfile', () => {
	it('sends DELETE request', async () => {
		...
		expect(mockFetch).toHaveBeenCalledWith('/api/profiles/abc-123', { method: 'DELETE' });
	});
