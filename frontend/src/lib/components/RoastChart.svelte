<script lang="ts">
	...
	function buildTempPath(points: TemperaturePoint[], accessor: (p: TemperaturePoint) => number): string {
	...
	function buildRorPath(points: TemperaturePoint[], accessor: (p: TemperaturePoint) => number): string {
	...
	function buildControlPath(points: ControlPoint[], accessor: (p: ControlPoint) => number): string {
		if (points.length === 0) return '';
		return points
			.map((p, i) => {
				const x = xScale(p.timestamp_ms);
	...
	// Temperature curves
	let etPath = $derived(options.showET ? buildTempPath(history, (p) => p.et) : '');
	let btPath = $derived(options.showBT ? buildTempPath(history, (p) => p.bt) : '');

	// RoR curves
	let etRorPath = $derived(options.showETRor ? buildRorPath(history, (p) => p.et_ror) : '');
	let btRorPath = $derived(options.showBTRor ? buildRorPath(history, (p) => p.bt_ror) : '');

	// Control curves
	let burnerPath = $derived(options.showBurner ? buildControlPath(controlHistory, (p) => p.burner) : '');
	let airflowPath = $derived(options.showAirflow ? buildControlPath(controlHistory, (p) => p.airflow) : '');
	let drumPath = $derived(options.showDrum ? buildControlPath(controlHistory, (p) => p.drum) : '');

	// Whether the right Y-axis is needed
	let showRightAxis = $derived(
		options.showETRor || options.showBTRor || options.showBurner || options.showAirflow || options.showDrum
	);
...
<div class="chart-container">
	<svg {width} {height} viewBox="0 0 {width} {height}">
		...
		{#if showRightAxis}
			<line x1={width - PADDING.right} y1={PADDING.top} x2={width - PADDING.right} y2={height - PADDING.bottom} stroke={AXIS_COLOR} stroke-width="1" />
			{#each [0, 25, 50, 75, 100] as val (val)}
				{@const y = yScaleRight(val)}
				<text x={width - PADDING.right + 8} y={y + 4} fill={TEXT_COLOR} font-size="9" text-anchor="start" opacity="0.6">{val}</text>
			{/each}
		{/if}

		<!-- Control curves (behind temperature curves) -->
		{#if burnerPath}
			<path d={burnerPath} fill="none" stroke={BURNER_COLOR} stroke-width="1" stroke-dasharray="none" opacity="0.4" />
		{/if}
		{#if airflowPath}
			<path d={airflowPath} fill="none" stroke={AIRFLOW_COLOR} stroke-width="1" stroke-dasharray="none" opacity="0.4" />
		{/if}
		{#if drumPath}
			<path d={drumPath} fill="none" stroke={DRUM_COLOR} stroke-width="1" stroke-dasharray="none" opacity="0.4" />
		{/if}

		<!-- RoR curves -->
		{#if etRorPath}
			<path d={etRorPath} fill="none" stroke={ET_ROR_COLOR} stroke-width="1.5" stroke-dasharray="4,3" />
		{/if}
		{#if btRorPath}
			<path d={btRorPath} fill="none" stroke={BT_ROR_COLOR} stroke-width="1.5" stroke-dasharray="4,3" />
		{/if}
		...
		{#each legendEntries as entry, i (entry.label)}
			...
			<rect x={lx} y={PADDING.top + 5} width="10" height="3" fill={entry.color} rx="1" />
			<text x={lx + 14} y={PADDING.top + 10} fill={entry.color} font-size="10">{entry.label}</text>
		{/each}
	</svg>
</div>
